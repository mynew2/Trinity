    ...
    public:
        ...
        class spell_icc_sprit_alarm_SpellScript : public SpellScript
        {
            ...
            void HandleEvent(SpellEffIndex effIndex)
            {
                ...
                if (GameObject* trap = GetCaster()->FindNearestGameObject(trapId, 20.0f))
                    trap->SetRespawnTime(trap->GetGOInfo()->trap.autoCloseTime);

                std::list<Creature*> wards;
                GetCaster()->GetCreatureListWithEntryInGrid(wards, NPC_DEATHBOUND_WARD, 20.0f);
                ...
                for (std::list<Creature*>::iterator itr = wards.begin(); itr != wards.end(); ++itr)
                {
                    if ((*itr)->IsAlive() && (*itr)->HasAura(SPELL_STONEFORM))
                    {
                        ...
                        if (Unit* target = (*itr)->SelectNearestTarget(20.0f))
